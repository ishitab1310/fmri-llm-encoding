...
print("="*60)
print("LLaDA Feature Extraction")
...
# ── Load annotation files once per language ───────────────────
ANN_CACHE = {}
for lang in args.langs:
    ...
    if os.path.exists(path):
        ...
        # onset/offset in seconds, section = run number 1-9
        ANN_CACHE[lang] = df
        print(f"  Annotation {lang}: {len(df)} words, "
              f"sections={sorted(df['section'].unique())}")
    else:
        print(f"  WARNING: no annotation for {lang} at {path}")
        ANN_CACHE[lang] = None

def build_windows(lang, run_num, n_trs):
    """Build one text string per TR using word_information.csv."""
    ...
    if run_df.empty:
        print(f"    WARNING: section {run_num} empty for {lang}")
        return ["[silence]"] * n_trs
    ...
    non_silence = sum(1 for w in windows if w != "[silence]")
    print(f"    section {run_num}: {n_trs} TRs, "
          f"{non_silence} with words ({len(words)} total words)")
    return windows
...
config_hf = AutoConfig.from_pretrained(
    args.model_dir, trust_remote_code=True, local_files_only=True)

model = AutoModel.from_pretrained(
    args.model_dir, config=config_hf,
...
print(f"Extracting layers: {LAYER_NAMES}")

_t = tokenizer("the little prince", return_tensors="pt").to("cuda")
with torch.no_grad():
    _o = model(**_t, output_hidden_states=True)
print(f"Sanity: {len(_o.hidden_states)} hidden states, "
      f"shape={_o.hidden_states[1].shape}  OK")
...
for lang in args.langs:
    ...
    for sub in SUBJECTS_BY_LANG[lang]:
        ...
        if not bolds:
            print(f"  [{sub}] SKIP — no BOLD files"); continue

        print(f"\n  {sub}: {len(bolds)} runs", flush=True)
        layer_runs = {name: [] for name in LAYER_NAMES}

        for run_i, bf in enumerate(bolds):
            # Extract section/run number from filename
            # e.g. run-15 → section 1, run-16 → section 2, etc.
            # The BOLD runs are numbered from 15+ for EN, but sections 1-9
            # Use run_i+1 as section number (1-indexed)
            run_num  = run_i + 1
            ...
            # Verify windows are not all silence
            n_words = sum(1 for w in windows if w != "[silence]")
            if n_words == 0:
                print(f"    run{run_i+1}: ALL SILENCE — check annotation")
        ...
        print(f"  Feature variance check (layer {LAYER_NAMES[4]}): {var:.4f}"
              f"  {'OK' if var > 1e-6 else 'WARNING: near-zero variance'}")

        json.dump({"sub": sub, "lang": lang, "layers": LAYER_NAMES,
                   "tr": TR, "n_runs": len(bolds)},
                  open(os.path.join(out_dir, "meta.json"), "w"), indent=2)
        print(f"  Saved {len(LAYER_NAMES)} layers -> {out_dir}", flush=True)
