...
Phase 1: English layer x lag search -> BEST_LAYER, BEST_LAG
...
# ── Shared constants (IDENTICAL to teammate) ──────────────────────────────────
TR, TRIM_TRS = 2.0, 4
...
# Lag grid — IDENTICAL to teammate (seconds)
LAG_GRID = {
    "lag024":   [0.0, 2.0, 4.0],
    "lag246":   [2.0, 4.0, 6.0],
...
def fit_loro_ridge(feat_runs, bold_runs, lag_seconds):
    ...
    if n_runs == 1:
        ...
        ridge.fit(Xtr, Y[:sp]); pred = ridge.predict(Xte)
        return np.array([
            pearsonr(Y[sp:, p], pred[:, p])[0]
            if Y[sp:, p].std() > 1e-8 and pred[:, p].std() > 1e-8
            else 0.
    ...
    for ti in range(n_runs):
        ...
        trY = np.concatenate([bold_runs[i] for i in range(n_runs) if i != ti])
        ...
        ridge = RidgeCV(alphas=RIDGE_ALPHAS, cv=RIDGE_CV)
        ridge.fit(trX, trY); pred = ridge.predict(teX)
        for p in range(n_parcels):
...
# ── Helpers: split raw features into per-run arrays ───────────────────────────
...
for sub in SUBJECTS["EN"]:
    ...
    for layer_name in layer_names:
        raw = np.load(os.path.join(args.feat_dir, "EN", sub, f"{layer_name}.npy"))
        ...
        for lag_name, lag_secs in LAG_GRID.items():
            ...
            en_rows.append({
                ...
                **{f"r_{roi}": float(r[i]) for i, roi in enumerate(PARCEL_KEYS)},
            })
...
if not en_df.empty:
    ...
    BEST_LAYER = "L20"; BEST_LAG = "lag024"; BEST_LAG_SECS = [0.,2.,4.]
    print("No EN results — using defaults")
...
# ══════════════════════════════════════════════════════════════════════════════
# PHASE 2: Population-level transfer EN -> FR
# ══════════════════════════════════════════════════════════════════════════════
print(f"\n{'='*60}")
...
for lang in ["EN", "FR"]:
    ...
    for sub in SUBJECTS[lang]:
        feat_path = os.path.join(args.feat_dir, lang, sub, f"{BEST_LAYER}.npy")
...
if not tr_df.empty:
    ...
    if   pct < 15:  print("  -> Low drop: LLaDA transfers well EN->FR")
